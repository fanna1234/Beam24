...
int main(int argc, char** argv) {
    ...
    std::vector<float> hZRealRow(szB);
    std::vector<float> hZImagRow(szB);
    ...
    std::vector<float> hCReal(szC, 0), hCImag(szC, 0);
    std::vector<float> hRefReal(szC, 0), hRefImag(szC, 0);
    ...
    for (int i = 0; i < warmup; ++i)
        sm120_sp_complex_f4_v10_half2_io_kernel<<<grid, block>>>(
    ...
    std::printf("\nKernel : %.9f ms  %.2f TOps/s (complex useful work, logical K=%d)\n", kern_ms, kern_tflops, K);
    std::printf(
        "{\"kind\":\"beam24_fused_complex_f4_v10_half2_io\",\"m\":%d,\"n\":%d,"
    ...
    CHECK_RT(cudaFree(dCReal)); CHECK_RT(cudaFree(dCImag)); CHECK_RT(cudaFree(dMeta));
