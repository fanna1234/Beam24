...
command -v flock >/dev/null || { echo "flock is required for the hierarchy campaign" >&2; exit 7; }
...
RUN_DIR="$(mktemp -d "$ROOT/artifact/runs/hierarchy.XXXXXX")"
...
for process in 0 1 2 3 4 5; do
  ...
  for position in 0 1; do
    ...
    case "$variant" in
      E)
        "$EXHAUSTIVE" 100 20 0 256 1024 1 0 > "$log" 2>&1
        expected=beam24_exhaustive_e2e
        ;;
      H)
        "$HIERARCHY" 100 20 0 256 1024 1 0 > "$log" 2>&1
...
"$PYTHON_BIN" "$ROOT/scripts/summarize_hierarchy.py" \
